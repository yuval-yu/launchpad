...
::: info
**版本** · v6（本版）：Envio 退回扫链工具，实体、Hasura 读路径、handler 里的取价 Effect 全部作废；Java 沿用现有的「Kafka → 审计表 → 投影」管线扩展。v5.1 及更早只在 git 历史里。
:::
...
- **Java**（launchpad · 消费、投影、口径、读接口）：沿用现有「监听 → 审计表 → 投影」管线加一组新 handler；事实表 + 派生表；线一定价、线二币视图、线三滚动窗口；读接口全查 MySQL。改造点见[第 5 页](/java)。

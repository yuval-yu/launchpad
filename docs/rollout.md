...
3. **P2 handler 与十二张表。** 新 topic、十个 handler；按[第 7 页](/tables)从零建表（V1 开头 DROP 全部旧表）；`PriceSource` 接口 + 路由 + `priceAt`；线二、线三。测试网灌数据，与链上 `balanceOf` / curve 储备对账。**出口**：一个币从发射到毕业后 Swap，所有表与链上一致。

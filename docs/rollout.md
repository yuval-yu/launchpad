...
| 配对资产名单与价源 | **09-22 改**：名单与美元价都取平台的配对资产接口 `/v1/quote-assets`，线一每分钟同步名单表、落价格行；原「稳定币写死 1 / 交易所 / 券商」三路价源与 `isStock`、`priceSymbol`、`STOCKS` 筛选全部删除，见[第 6 页](/pricing) |
...
| Q7 | **test / prod 的配对资产接口地址** | 平台 | 未给之前 test 暂用 devnet 占位（test 是主网，地址对不上，上 test 前必须换）；prod 不配就启动失败 |
...
| **配对资产接口不可用** | 那一轮名单与价格都不写，沿用库里的名单与上一个价 | 不做主备（用户 09-22 定：成功一次就都在库里）；价格比区块时间旧超过一小时打 WARN |
| **链上注册了新配对资产，平台接口还没给** | 发币消息 FAILED（没有精度就换不出整枚） | 接口一给、线一下一轮同步进名单后自动重投 |

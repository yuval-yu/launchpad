...
3. **P2 handler 与十张表。** 新 topic、九个 handler；按[第 7 页](/tables)从零建十一张 `launchpad_v2_*` 表（旧表不动）；`PriceSource` 接口 + 路由 + `priceAt`；线二、线三。测试网灌数据，与链上 `balanceOf` / curve 储备对账。**出口**：一个币从发射到毕业后 Swap，所有表与链上一致。

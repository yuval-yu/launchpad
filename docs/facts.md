...
| ERC20 语义：余额累加、销毁减供应、持有人数 | ~~Envio 维护内部 `Balance`，消息给变动后的绝对值，Java 只 set~~ **09-21 改：Java 从转账事实行累加**（首插才原子加减），Envio 只给 `from` / `to` / `value` 与地址类别。这是「链上语义归扫链」唯一让出来的一块，原因是扫链那边算太慢；Java 不靠查链算余额；09-21 另加了一个**校正**任务，拿链上的 `balanceOf` 核对自己累加出来的数（只在 prod 开），这是 Java 里第二处、也是最后一处查链 |

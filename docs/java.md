...
| `service/chain/handler/*` | **新** | 十一种事件的 handler，见下 |
...
| V4PoolGraduated | `launchpad_pool` upsert | 池行全部初值（储备、L、价格、positionId）；币行 `pool_id` `pool_created_at` `price_quote` | — |
| PoolRegistered | `launchpad_pool` upsert | 池行 `quote_asset` `registered_at`；币行 `pool_id` | — |
| ModifyLiquidity | — | 池行 `liquidity` `quote_reserve` `token_reserve` | — |
| LaunchGraduationRescued | — | 币行 `rescued_at` `status` | — |
| Swap | `launchpad_trade` | 池行 `liquidity` `quote_reserve` `token_reserve` `price_quote` `last_swap_at`；币行 `price_quote` `last_trade_at` | 同曲线成交；池行 `swap_count` / `cum_volume_quote`；trader 为 null 不进 position |
...
| **二 · 币视图** | 币行 + 余额表 + 价格表最新行 | `price_usd = price_quote × 配对资产现价`、`market_cap_usd = price_usd × total_supply`、`liquidity_usd`（曲线：`quote_reserve` × 配对资产价 × 2；毕业后：`pool.quote_reserve` × 配对资产价 + `pool.token_reserve` × `price_usd`）、`creator_holding_pct`；新绑定钱包的发行者补 `creator_user_id` | 币行口径列 | 每分钟，一条 UPDATE 全表 |

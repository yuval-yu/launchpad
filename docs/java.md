...
**读接口前面先不加缓存**（用户 09-21 定）：每次请求直接查 MySQL，上线前看压测数据再决定哪些接口值得加几秒的短缓存。保护第三方调用的缓存不在此列（持有者页签换表之前的 CMC 缓存、配对资产余额查链的 30 秒缓存）。

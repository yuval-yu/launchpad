...
**不做入库前过滤。** 工厂发的全收，没有「是不是我们的币」的判断。**认币**：先读 `payload.token.token`（扫链已带）；没有再用自己的表反查——曲线事件用 `payload.address` 查 `curve_address`，Swap 用 `args.id` 查 `pool_id`，Transfer 的 `payload.address` 就是 token。查不到 = 发币消息还没到 → WAITING_TOKEN，TokenLaunched 投影后按币重投。

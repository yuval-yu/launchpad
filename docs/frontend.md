...
| `GET /coin/kline` | M5 `launchpad_v2_trade` 逐笔；H1 / H6 / D1 `launchpad_v2_kline_minute`；ALL `launchpad_v2_kline_hour` | 无。五档窗口与颗粒不变；M5 每点 `priceUsd` = `price_quote × quote_usd_price`（成交行上两列都有），桶档取 `close_usd`；`marketCapUsd` = `priceUsd × total_supply`；`stale` 恒 false |
| `GET /coin/trades` | `launchpad_v2_trade` 按币倒序 | 游标从 CMC 的 lastId 换成 `(block_time, id)` 编码的不透明串，契约不变；`exchange` 由 `venue` 映射（CURVE →「Bonding Curve」，POOL →「Uniswap v4」）；`quoteAsset` ← 币行 `quote_asset_symbol`（名单外为 null）；`tokenAmount` = `token_amount` ÷ 10^`token_decimals`、`quoteAmount` ← `quote_amount_whole`、两个 Raw ← 原值；`amountUsd` 可为 null；`traderAddress` 池内可为 null；`stale` 恒 false、`fetchedAt` = 响应时刻 |
| `GET /coin/holders` | `launchpad_v2_balance` 按币余额倒序 | **`publicName` 与 `tags` 恒为 null**；`bondingCurve` ← `holder_kind = CURVE`，其余非 USER 行不列；`creator` ← `holder_address = 币行 creator_address`；`pct` = `balance` ÷ `total_supply`；`balance` = `balance` ÷ 10^`token_decimals`，`balanceRaw` 原值；`rank` 读时编号；`holderCount` = 币行 `holder_count` − 该币非 USER 且余额 > 0 的行数（最多几行，一次 COUNT） |
| `GET /assets/activity` | `launchpad_v2_trade` 按 trader | 无。链上直连的成交也会出现；`amountUsd` 可为 null。**`priceQuote` ← `avg_price_quote`**（这笔的成交均价，旧表就是这个含义），不是成交后边际价 `price_quote`；`quoteAmountWhole`、`side`、`venue`、`logIndex`、`traderAddress` 同名直取；币名、代号、图从币行补 |

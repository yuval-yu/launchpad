...
| `GET /coin/detail` | `launchpad_token` + `launchpad_coin_price` 最新行 + 绑定 + 用户 | 无。不再打开时同步刷；`liquidityUsd = liquidity_quote × 配对资产价` |
| `GET /coin/kline` | M5 `launchpad_trade` 逐笔；H1 / H6 / D1 `launchpad_kline_minute`；ALL `launchpad_kline_hour` | 无。五档窗口与颗粒不变；`stale` 恒 false |
...
| ALL | 自发射起 | `launchpad_kline_hour` 按跨度合并成 2h / 12h / 1d / 1w / 1M；一年 8,760 行封顶，不建日桶 |

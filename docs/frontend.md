...
| `GET /coin/trades` | `launchpad_v2_trade` 按币倒序 | 游标从 CMC 的 lastId 换成 `(block_time, id)` 编码的不透明串，契约不变；`exchange` 由 `venue` 映射（CURVE →「Bonding Curve」，POOL →「Uniswap v4」）；`quoteAsset` ← 币行 `quote_asset_symbol`（名单外为 null）；`tokenAmount` ← `token_amount`、`quoteAmount` ← `quote_amount`（库里就是整枚）、两个 Raw ← 读时还原成最小单位（`× 10^精度`，精确）；`amountUsd` 可为 null；**`tradeKind`（09-23 新增）** = `USER` / `BUYBACK` / `FEE_CONVERSION`，后两种是运营清扫毕业池手续费时 hook 在池内做的内部 Swap（库里 `trader_address` 为空，读时按买卖方向派生：买 = 回购、卖 = 手续费兑换），这时 `traderAddress` 兜底给成交行上的 hook 地址而不是 null，前端按 `tradeKind` 渲染标签；`stale` 恒 false、`fetchedAt` = 响应时刻 |
| `GET /coin/holders` | `launchpad_v2_balance` 按币余额倒序 | **`publicName` 与 `tags` 恒为 null**；`bondingCurve` ← `holder_kind = CURVE`，其余非 USER 行不列；`creator` ← `holder_address = 币行 creator_address`；`pct` = `balance` ÷ `total_supply`；`balance` ← `balance`（库里就是整枚），`balanceRaw` 读时还原成最小单位；`rank` 读时编号；`holderCount` = 币行 `holder_count` − 该币非 USER 且余额 > 0 的行数（最多几行，一次 COUNT） **09-21 实现口径**：只取 `balance > 0`，只列 USER 与 CURVE（两类分开查再合并，走 `(token_address, holder_kind, balance DESC)` 索引）；总数直接取币行 `holder_count`（它只数用户地址，不再减）；`pct` = 余额 ÷ `total_supply`，百分数、8 位、向下截断，总供应为空则 null；同额按地址升序兜底；`tags` 为 null（不是空数组） **09-23 追加**：新增 `holderKind`（`USER` / `CURVE` / `POOL_MANAGER`，`bondingCurve` 保留不动），并**放开 `POOL_MANAGER`**——毕业后曲线余额归 0，整块流动性转进 Uniswap v4 的 PoolManager 单例，只查 USER + CURVE 的话榜单只剩散户、占比之和差着一大块（dev 上一个已毕业的币差 23%）。三类分开查再合并，索引与原来一样。金库 `VAULT`（回购锁仓）与毕业 Hook（归 `RECEIVER`，池内待结算费用）也有真实余额，用户 09-23 定**只放开池子**，那两类继续不列；`holder_count` 口径不变（只数用户，所以榜单多一行曲线或池子、与总数对不上是正常的） |
...
| `GET /assets/positions`（新） | `launchpad_v2_position` + `launchpad_v2_balance` + `launchpad_v2_token` | **新接口**。币、数量（余额表）、下单均价、当前价、未实现盈亏 % 与金额（币本位与 USD）、累计买卖、已实现盈亏；~~持有市值低于粉尘阈值的行不列~~（用户 09-21 定：粉尘过滤先不做，余额 > 0 就列） **09-21 实现口径**：以余额表为主——视角地址余额 > 0 的每个币一行，有余额没成交（转入 / 空投 / 野池买来的）也列、成本类字段为空；同一个币在本人的两个地址上是两行，不合并。字段：`tokenAddress` `name` `symbol` `imageUri` `quoteAssetSymbol` `holderAddress` `holdingQty`（余额）`valueUsd` `tradedQty`（由成交推出的持有量）`avgPriceQuote` `avgPriceUsd` `currentPriceQuote` `currentPriceUsd` `unrealizedPnlQuote` `unrealizedPnlUsd` `unrealizedPnlPct` `boughtQty` `boughtQuote` `boughtUsd` `soldQty` `soldQuote` `soldUsd` `realizedPnlQuote` `realizedPnlUsd`；顶层 `{chainId, syncedAt, items}`。一律整枚、没有 Raw 字段、数字一律字符串。未实现盈亏 = `(当前价 − 均价) × min(holdingQty, tradedQty)`（转入来的那部分没有成本，不算盈亏），任一输入为空则为空；`unrealizedPnlPct` 跟配对资产口径。按市值倒序（空值最后）、不分页、上限 200、无缓存 |
...
| `GET /assets/balances/tokens` | `launchpad_v2_balance` 按 holder，`holder_kind = USER` 且 `balance > 0`（余额由 Java 累加，可能短暂为负，读侧只取大于 0），只留币行里有的币 | 无。`balanceDecimal` ← `balance`（库里就是整枚）、`balance` 字符串读时还原成最小单位；`stage` ← 币行 `status`、`priceUsd` / `marketCapUsd` / `priceChange24h` ← 币行；`valueUsd` 读时算；**每行与顶层 `syncedAt` 都取 `launchpad_v2_indexer_state.processed_block_time`**（余额截至 Envio 处理到的区块，余额表本身不存时间） **⚠️ 需要知会前端（09-21）：`priceChange24h` 的单位变了**——旧表是小数（0.0208），现在取币行 `price_change_24h`，是百分比数（12.71 = +12.71%），与市场卡片同源；字段名与类型不变。`syncedAt` 取**消费水位**（本服务消费到的最大区块的时间；09-21 去掉 Heartbeat 后改的），市场安静时停在最后一条消息的区块上，一条消息都没消费过时为 null；上限 200 行 |
